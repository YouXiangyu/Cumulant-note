...
- 收集箱整理状态面板与恢复动作第一版：集中展示 listener、queue、worker、conflict、movement log 的状态、最近事件和最近错误；主操作区提供导入、递归扫描入队、生成计划、运行整理、恢复并运行 worker 和启动/停止监听，状态面板保留失败/冲突队列项单项重试或跳过、冲突规则处理和 movement log 单项回滚。
...
当前代码已经具备收集箱递归展示、递归扫描入队、监听入队、移动、空目录清理、日志、回滚、手动触发的单并发队列消费能力、冲突问答与规则记忆第一版，以及收集箱状态面板和单项恢复动作第一版；动态归档模板 / Archive Map 尚未实现，当前整理计划仍主要依赖单文件抽取和 MiMo 目标路径决策；长期常驻自动整理、长时间实机监听稳定性、批量处理策略和更完整的自动冲突处理仍是后续目标。
...
- `src-tauri/src/services/archive_map.rs`（待实现）：扫描正式 Vault 目录结构，生成动态归档模板 / Archive Map，供整理计划和 worker 共享目标目录上下文。
...
MiMo 是 AI provider，负责抽取、理解、整理建议和回答生成；listener 只负责发现 `000-收集箱/` 中允许处理的普通文件并稳定去重入队，递归扫描和递归 watcher 会覆盖子目录，但会跳过 ledger、`.thebrain`、`.secrets`、隐藏/临时文件和不支持的文件类型；后台 worker 是本地调度器，负责从队列取任务、稳定等待、预算检查、调用 MiMo、执行移动、写日志、重试和回滚。当前已接入第一层手动 drain worker：它只处理 `000-收集箱/` 中的普通文件；只有抽取和整理决策均为 `ok` 且 `is_mock=false` 时才会移动文件，移动成功后会清理收集箱内变空的父目录。冲突规则第一版只做推荐与用户确认应用，规则 Markdown 位于 `.thebrain/rules/`，不会被 listener 或 RAG 当作用户内容处理。收集箱状态面板第一版会把 listener、queue、worker、conflict 和 movement log 汇总到同一页面，并提供单项重试、跳过和回滚。长期常驻 worker、批量策略和完整冲突恢复体验仍是后续目标。
...
- 动态归档模板 / Archive Map：扫描正式 Vault 目录结构，完全排除 `000-收集箱/` 及其子目录，生成可读归档地图，并让“生成计划”和后台 worker 优先按已有目录体系选择目标路径。
...
当前 Rust 单元测试覆盖 Vault 初始化、路径安全、Markdown frontmatter、ledger、SQLite schema、收集箱递归展示、listener 递归扫描/跳过规则/稳定等待/去重、队列 claim/retry/单项跳过、预算、audit、导入、导入队列项 worker 处理、MiMo fallback、worker 暂停与失败不移动、移动/空目录清理/回滚、冲突规则 Markdown 写入、相似规则推荐、规则应用不覆盖、规则路径不被 listener 处理、TODO/日程候选、便利贴和 RAG 基础能力。
