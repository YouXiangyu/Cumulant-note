...
- `src/api.ts`：封装 Tauri commands，暴露受控导入、收集箱 listener、MiMo 状态、抽取、整理计划、worker、resident worker、queue 单项/批量恢复、移动、单项/批量回滚、audit timeline、冲突、冲突规则和 RAG 命令。
- `src/styles.css`：视觉 token、左侧导航、页面网格、收集箱、RAG 面板、Markdown 三栏编辑器和便利贴布局。

本地后端服务层：

- `src-tauri/src/commands.rs`：暴露 Vault、Markdown、收集箱、ledger、导入、MiMo、listener、队列、worker、resident worker、预算、移动/回滚、audit timeline、候选、便利贴、快捷键、冲突和 RAG commands。
